...
$$
y^{(m)}=X^{(m)}\beta^{(m)}+\epsilon^{(m)} (\#eq:eq1)
$$
...
$$
\beta=\textrm{argmin}\{L(X,y;\beta)+P(\beta;\lambda)\} (\#eq:eq2)
$$
...
<p class="caption">(\#fig:p7-1)数据结构</p>
</div>
...
$$
P(\beta;\lambda,\gamma)=\lambda\sum_{j=1}^p ||\beta_j||^\gamma=\lambda\sum_{j=1}^p((\sum_{i=1}^M (\beta_j^{(i)})^2)^{1/2})^\gamma (\#eq:eq3)
$$
...
$$
P(\beta;\lambda,a)=\sum_{j=1}^p P_{MCP}(||\beta_j||;\lambda,a) (\#eq:eq4)
$$
...
$$
P(\beta;\lambda)=\lambda\sum_{j=1}^p ||\beta_j|| (\#eq:eq5)
$$
...
$$
P(\beta;\lambda,a)=\sum_{j=1}^p P_{MCP}(||\beta_j||_1;\lambda,a) (\#eq:eq6)
$$
...
$$
P(\beta;\lambda,a,b)=\sum_{j=1}^p P_{MCP}(\sum_{m=1}^M P_{MCP}(|\beta_j^{(m)}|;\lambda,a);\lambda,b) (\#eq:eq7)
$$
...
P_{adSGL}(\beta;\lambda_1,\lambda_2)=\lambda_1\sum_{j=1}^p w_j||\beta_j||_2+\lambda_2\xi'|\beta| (\#eq:eq9)
$$
...
$$
P_C(\beta)=\lambda\sum_{j=1}^p \sum_{k \neq l} a_j^{(kl)}(\beta_j^{(k)}-\beta_j^{(l)})^2 (\#eq:eq11)
$$
